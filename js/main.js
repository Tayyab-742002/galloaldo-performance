...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Cap the fonts.ready wait at 100 ms — font-display:swap means fallback fonts
  // show immediately so there is no FOIT risk. Clearing __plTimer cancels the
  // CSS-class fallback in bodyscript.html so the full GSAP animation plays on
  // fast connections while slow mobile gets an instant CSS-transition reveal.
  var fontsStarted = false;
  function startReveal() {
    if (fontsStarted) return;
    fontsStarted = true;
    if (window.__plTimer) { clearTimeout(window.__plTimer); window.__plTimer = null; }
    initComponents();
    initialReveal();
  }
...
const PageReveal = (function() {
  ...
  function init(tl) {
    ...
    tl.add(() => {
      if (MainSlider1.isActive()) {
        MainSlider1.autoplayStart();
      }
    })
...
//ADD
    ...
    let sliderAutoplayStartDelay = 50000;
...
/*--------------------------------------------------
  07b. Responsive background images (mobile/desktop)
---------------------------------------------------*/

function setResponsiveBackgrounds() {
  if (window.innerWidth > 767) return;

  document.querySelectorAll('[data-background-mobile]').forEach(function(el) {
    var mobileSrc = el.getAttribute('data-background-mobile');
    if (mobileSrc) {
      el.setAttribute('data-background', mobileSrc);
    }
  });
}
